...
const getList = async () => {
  loading.value = true
  try {
    const res = await getDirectorList(queryParams)
...
const handleEdit = (row) => {
  dialogTitle.value = '编辑导演'
  Object.assign(form, row)
  dialogVisible.value = true
}

const handleDelete = (row) => {
  ElMessageBox.confirm(`确认删除导演 "${row.name}" 吗?`, '提示', { type: 'warning' }).then(
    async () => {
      await deleteDirector(row.id)
...
const submitForm = async () => {
  ...
  try {
    if (form.id) await updateDirector(form)
    else await addDirector(form)
    ...
    <el-card shadow="never" class="mb-20">
      <el-form :inline="true">
        ...
        <el-form-item>
          ...
          <el-button type="success" :icon="Plus" @click="handleAdd">新增导演</el-button>
        </el-form-item>
      </el-form>
    </el-card>
